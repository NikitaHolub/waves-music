  ...
  <AppBar position="static">
    <Toolbar>
      ...
      <div>
        ...
        <Link to="/shops/all">
          <Button style={isActive(history, "/shops/all")}>Список категорий</Button>
        </Link>
        <Link to="/cart">
      ...
      <div style={{'position':'absolute', 'right': '10px'}}><span style={{'float': 'right'}}>
      {
        !auth.isAuthenticated() && (<span>
          <Link to="/signup">
            <Button style={isActive(history, "/signup")}>Зарегестрироваться
            </Button>
          </Link>
          <Link to="/signin">
      ...
      {
        auth.isAuthenticated() && (<span>
          {auth.isAuthenticated().user.seller && (<Link to="/seller/shops"><Button style={isPartActive(history, "/seller/")}>Мои товары</Button></Link>)}
